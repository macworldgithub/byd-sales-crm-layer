...
export function Header({ onSelectCustomer, onOpenQuickDeal }: HeaderProps) {
  const {
    currentUser,
    setCurrentRole,
    currentRole,
  ...
  return (
    <header className="topbar">
      {/* Left: Branding Kicker & Site Switcher */}
      <div className="flex items-center gap-4">
        <div>
          <span className="topbar-kicker flex items-center gap-1.5 font-mono">
          ...
          <h2 className="text-base font-semibold text-slate-900 tracking-tight flex items-center gap-2">
            <span>Harmony Auto Group</span>
            <span className="text-slate-300">/</span>
            <span className="text-[#e60012]">{selectedSite}</span>
          </h2>
        </div>

        {/* Site Selector Dropdown */}
        <div ref={siteMenuRef} className="relative hidden md:block">
          <button
            onClick={() => setIsSiteMenuOpen(!isSiteMenuOpen)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100/80 hover:bg-slate-200/70 border border-slate-200 text-xs font-semibold text-slate-700 transition-all"
          >
      ...
      {/* Center: Global Customer 360 Search (§5.1 & AC-1) */}
      <div ref={searchRef} className="relative flex-1 max-w-lg mx-4 hidden lg:block">
        ...
        {isSearchOpen && q.length >= 2 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 max-h-[460px] overflow-y-auto p-3 z-50 divide-y divide-slate-100">
            {totalResults === 0 ? (
                        ...
                        >
                          <div>
                            <div className="text-xs font-bold text-slate-900 flex items-center gap-2">
                              <span>{o.vehicle_descriptor}</span>
      ...
      {/* Right Controls: Role Switcher, Online Status & User Profile */}
      <div className="flex items-center gap-3">
        ...
        <div ref={userMenuRef} className="relative">
          <button
            ...
            className="flex items-center gap-2.5 p-1.5 pr-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all shadow-sm"
          >
            <div className="w-7 h-7 rounded-lg bg-[#171b22] text-white flex items-center justify-center font-bold text-xs font-mono">
